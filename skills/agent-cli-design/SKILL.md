...
**3. 命令声明是 help 与补全的唯一真相源。** 在注册命令的地方同时声明命令名、argument、option、
默认值、必填性和一行说明，让框架从这些元数据生成标准 `Usage / Commands / Arguments / Options`
help。不要另建手写 help map、手动分流 `--help` / `--version`，也不要维护一份与真实 parser
分离的命令文档；两套声明迟早漂移。每个可见命令、argument 和 option 都必须有说明。复杂命令才在
leaf help 追加少量例子。

顶层 help 是导航，不是手册：只负责让 agent 快速选对下一条命令，列 usage 形状、主要命令、全局
输出/目标 flag，以及「继续看哪个子命令 help」的指针。不要在顶层内联某个 workflow 的全部细节；
group help 一行一个子命令，leaf help 才放 required flags、局部语义、副作用和少量例子。无参数运行
时显示顶层 help。

同一份命令元数据还应驱动 shell completion。优先用框架/生成器遍历命令树生成 bash、zsh、fish
补全，不要另写一份命令列表。安装器可以把 completion 文件放进 shell 的标准补全目录；若必须修改
用户的 shell profile，只能通过显式安装步骤执行，并做到幂等、带清晰标记、可卸载，不能在普通
CLI 运行时静默注入。

**4. 短参数只给高频、安全、无歧义的操作。** 长参数是稳定、自解释的 API；短参数是热路径上的
输入优化，不是每个 option 都必须拥有的装饰。先统计高频调用，再分配短名，并遵守：

- 保留惯例：`-h, --help`；version 优先采用生态惯例（Commander 常用 `-V, --version`）。`-v`
  留给 verbose 时，不要让同一 CLI 中的 `-v` 同时表示 version 和 verbose。
- 高频输出与路径适合短名，如 `-j, --json`、`-o, --output`、`-p, --path`；领域筛选器可用首字母，
  如 `-t, --type`、`-g, --grep`，但只在含义稳定且作用域内无冲突时分配。
- 危险确认和低频语义保留长拼写，例如 `--apply`、`--force`、`--overwrite`、`--discard-local`。
  长拼写迫使调用者看见意图，也避免一字符误触。
- 在同一个 command 作用域内保证短名唯一；共享 option mixin 必须集中声明短名。不同 leaf 可复用
  同一短名，但同一概念跨命令应保持同一短名。
- 不为了追求短而新增命令别名。命令别名会扩大词汇表和补全表；只有真实、高频且长期稳定的用法才
  值得引入。
- 测试每个短参数与长参数完全等价，并验证 help、completion 和 parser 都来自同一声明。

**5.「一条命令覆盖一片」省的是 *context*，不是调用次数。** 这是批量操作对 agent 重要的最深
理由。N 条单点命令的循环会产生 N 份结果回显 *堆进 context window* —— N 份几乎一样的输出。
一条带 selector 的命令把它压成一份命中摘要。你省的不是 5 秒，是 50 份 context。

**6. 错误即下一条命令。** 出错时，错误体里应当装着 *修法*，而不只是诊断：
- 命中歧义 → 回候选地址 + 消歧的 flag
- 无命中 → 回当前有效的区间/范围
- 缺/多参数 → 回那条改好的调用
- 前置条件未满足 → 回该先跑什么
...
**7. 护栏住在工具里，不住在文档里。** agent 会 *自信地宣称成功* ——「我已经把所有标记清掉
